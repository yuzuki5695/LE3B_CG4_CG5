...
void DsvManager::Initialize(DirectXCommon* directXCommon) {
    ...
    descriptorHeap = dxCommon_->CreateDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_DSV, 1, false);
    // DescriptorSizeを取得する
    descriptorSize =  dxCommon_->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
    ...
    // DSVDescの先頭にDSVを作る
    dxCommon_->GetDevice()->CreateDepthStencilView(depthStencilResource.Get(), &dsvDesc, descriptorHeap->GetCPUDescriptorHandleForHeapStart());
